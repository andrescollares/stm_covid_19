...
class TokenizerMapper3 extends Mapper[Object, Text, Text, IntWritable] {
  val one = new IntWritable(1)
  val day_usertype = new Text
  ...
  override def map(
    ...
    try {
      ...
      day_usertype.set(
        row_elems(2).takeWhile(_ != 'T') + "_" + row_elems(8).replaceAll(" ", "-")
      )
    } catch {
      case e: ArrayIndexOutOfBoundsException => day_usertype.set("NONE")
    } finally {
      context.write(day_usertype, one)
    }
  }
}
...
class IntSumReducer3 extends Reducer[Text, IntWritable, Text, IntWritable] {
  override def reduce(
      key: Text,
...
object CityMovementUsertype {
  ...
  def main(args: Array[String]): Unit = {
    ...
    job.setJarByClass(classOf[TokenizerMapper3])
    job.setMapperClass(classOf[TokenizerMapper3])
    job.setCombinerClass(classOf[IntSumReducer3])
    job.setReducerClass(classOf[IntSumReducer3])
